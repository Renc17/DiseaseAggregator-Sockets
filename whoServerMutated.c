...
int main(int argc, char** argv){
    ...
    printf("Main Thread just set up server for statistics and query\n");
                ...
                if(FD_ISSET(j, &branchStats)){
                    ...
                    if (len > 0) {
                        ...
                        if (i == 0) {
                            ...
                            pthread_mutex_lock(&printMutex);
                            printf("Thread %ld WorkerPort %s. Position %d\n", pthread_self(), message, workerCount);
                            pthread_mutex_unlock(&printMutex);
